...
if ($result_platillos->num_rows > 0) {
    ...
    while($row = $result_platillos->fetch_assoc()) {
        echo "<tr>
                ...
                <td>\${$row['precio']}</td>
                <td>
                <form action='editar_platillo.php' method='get' style='display:inline-block;'>
    <input type='hidden' name='id' value='{$row['id_platillo']}'>
    <button type='submit'>Modificar</button>
</form>
            <form action='conexion.php' method='post' style='display:inline-block;' onsubmit=\"return confirm('¿Estás seguro de eliminar este platillo?');\">
    <input type='hidden' name='id' value='{$row['id_platillo']}'>
    <button type='submit'>Eliminar</button>
</form>
                </td>
...
echo "<br><br><button onclick=\"window.location.href='addplatillo.php'\" style=\"padding: 10px; 20px\">Agregar Nuevo Platillo</button>";
...
echo "<tr>
    ...
    while($row = $result_postre->fetch_assoc()){
        echo "<tr>
                ...
                <td>{$row['precio']}</td>
                <td>
                <form action='editar_postre.php' method='get' style='display:inline-block;'>
    <input type='hidden' name='id' value='{$row['id_postre']}'>
    <button type='submit'>Modificar</button>
</form>
            <form action='conexion.php' method='post' style='display:inline-block;' onsubmit=\"return confirm('¿Estás seguro de eliminar este postre?');\">
    <input type='hidden' name='id' value='{$row['id_postre']}'>
    <button type='submit'>Eliminar</button>
</form>
                </td>
...
echo "<br><br><button onclick=\"window.location.href='addpostre.php'\" style=\"padding: 10px 20px;\"> Agregar Nuevo Postre</button>";
